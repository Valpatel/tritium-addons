        ...
        window._tritiumEventBus.emit("panel:register-tab", {
            ...
            create: function(el) {
                el.innerHTML = "<div style=\"padding:8px;font-family:monospace;font-size:11px;color:#ccc\">"
                    + "<div style=\"color:#4a154b;font-size:12px;margin-bottom:8px\">SLACK</div>"
                    + "<p style=\"color:#555;font-size:10px;margin-bottom:10px\">Slack workspace integration</p>"
                    + "<div style=\"color:#888;margin-bottom:6px;font-size:10px\">CONFIGURATION</div>"
                    + "<div class=\"cf-row\"><label class=\"cf-label\">WEBHOOK_URL</label><input type=\"text\" class=\"cf-input\" data-key=\"webhook_url\" placeholder=\"Incoming webhook URL\"></div><div class=\"cf-row\"><label class=\"cf-label\">BOT_TOKEN</label><input type=\"text\" class=\"cf-input\" data-key=\"bot_token\" placeholder=\"Bot OAuth token\"></div><div class=\"cf-row\"><label class=\"cf-label\">CHANNEL</label><input type=\"text\" class=\"cf-input\" data-key=\"channel\" placeholder=\"Default channel\"></div><div class=\"cf-row\"><label class=\"cf-label\">BRIDGE_ALERTS</label><input type=\"checkbox\" class=\"cf-input\" data-key=\"bridge_alerts\"></div>"
                    + "<div style=\"margin-top:10px;display:flex;gap:4px\">"
                    + "<button class=\"cf-btn\" data-action=\"save\" style=\"color:#4a154b;border-color:#4a154b\">SAVE</button>"
                ...
                if (save) save.onclick = function() {
                    ...
                    fetch("/api/comms/slack/config", {method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})
                ...
                if (test) test.onclick = function() {
                    fetch("/api/comms/slack/test",{method:"POST"}).then(function(r){return r.json();}).then(function(d){
                ...
                if (en) en.onclick = function() {
                    fetch("/api/comms/slack/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:true})})
