# Slack Bridge — comms addon

> Parent: [tritium-addons](../README.md) · Archetype: [COMMS-BRIDGES.md](../COMMS-BRIDGES.md)

**What it's for:** Post alerts to Slack channels and receive commands via slash commands.

**Status:** **Stub.** `SlackPlugin.send_message()` logs "Would send" and returns `True` without connecting. It is loaded and `start()`ed by the dispatcher, but never actually invoked — the fan-out only calls plugins that define `send_message_sync`, and this one doesn't (see [COMMS-BRIDGES.md → D3](../COMMS-BRIDGES.md#reality-check)). No tests.

## Wire

- **Entry class:** `SlackPlugin` in [`plugin.py`](plugin.py) — a bare, duck-typed
...
- **Routes:** [`routes.py`](routes.py) declares `/api/comms/slack`
...
## Config keys (`SlackPlugin._config`)
...
Use an incoming-webhook URL, or `slack_sdk` for a full bot. Then **add a synchronous `send_message_sync(text, payload)`** — without it the dispatcher loads and `start()`s the plugin but never calls it. Follow [COMMS-BRIDGES.md → Making a stub real](../COMMS-BRIDGES.md#making-a-stub-real).
