...
// Webhooks communication addon tab
        ...
        window._tritiumEventBus.emit("panel:register-tab", {
            ...
            create: function(el) {
                el.innerHTML = "<div style=\"padding:8px;font-family:monospace;font-size:11px;color:#ccc\">"
                    + "<div style=\"color:#ff9900;font-size:12px;margin-bottom:8px\">WEBHOOKS</div>"
                    + "<p style=\"color:#555;font-size:10px;margin-bottom:10px\">Generic webhook POST to any URL</p>"
                    + "<div style=\"color:#888;margin-bottom:6px;font-size:10px\">CONFIGURATION</div>"
                    + "<div class=\"cf-row\"><label class=\"cf-label\">WEBHOOK_URL</label><input type=\"text\" class=\"cf-input\" data-key=\"webhook_url\" placeholder=\"Target URL\"></div><div class=\"cf-row\"><label class=\"cf-label\">SECRET</label><input type=\"text\" class=\"cf-input\" data-key=\"secret\" placeholder=\"HMAC secret\"></div><div class=\"cf-row\"><label class=\"cf-label\">EVENTS</label><input type=\"text\" class=\"cf-input\" data-key=\"events\" placeholder=\"Events to forward\"></div><div class=\"cf-row\"><label class=\"cf-label\">FORMAT</label><input type=\"text\" class=\"cf-input\" data-key=\"format\" placeholder=\"Payload format\"></div>"
                    + "<div style=\"margin-top:10px;display:flex;gap:4px\">"
                    + "<button class=\"cf-btn\" data-action=\"save\" style=\"color:#ff9900;border-color:#ff9900\">SAVE</button>"
                ...
                if (save) save.onclick = function() {
                    ...
                    fetch("/api/comms/webhooks/config", {method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})
                ...
                if (test) test.onclick = function() {
                    fetch("/api/comms/webhooks/test",{method:"POST"}).then(function(r){return r.json();}).then(function(d){
                ...
                if (en) en.onclick = function() {
                    fetch("/api/comms/webhooks/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:true})})
