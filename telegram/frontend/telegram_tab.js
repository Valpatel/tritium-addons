...
// Telegram communication addon tab
        ...
        window._tritiumEventBus.emit("panel:register-tab", {
            ...
            create: function(el) {
                el.innerHTML = "<div style=\"padding:8px;font-family:monospace;font-size:11px;color:#ccc\">"
                    + "<div style=\"color:#0088cc;font-size:12px;margin-bottom:8px\">TELEGRAM</div>"
                    + "<p style=\"color:#555;font-size:10px;margin-bottom:10px\">Telegram Bot API bridge</p>"
                    + "<div style=\"color:#888;margin-bottom:6px;font-size:10px\">CONFIGURATION</div>"
                    + "<div class=\"cf-row\"><label class=\"cf-label\">BOT_TOKEN</label><input type=\"text\" class=\"cf-input\" data-key=\"bot_token\" placeholder=\"Bot token from BotFather\"></div><div class=\"cf-row\"><label class=\"cf-label\">CHAT_ID</label><input type=\"text\" class=\"cf-input\" data-key=\"chat_id\" placeholder=\"Target chat/group ID\"></div><div class=\"cf-row\"><label class=\"cf-label\">BRIDGE_ALERTS</label><input type=\"checkbox\" class=\"cf-input\" data-key=\"bridge_alerts\"></div><div class=\"cf-row\"><label class=\"cf-label\">BRIDGE_COMMANDS</label><input type=\"checkbox\" class=\"cf-input\" data-key=\"bridge_commands\"></div>"
                    + "<div style=\"margin-top:10px;display:flex;gap:4px\">"
                    + "<button class=\"cf-btn\" data-action=\"save\" style=\"color:#0088cc;border-color:#0088cc\">SAVE</button>"
                ...
                if (save) save.onclick = function() {
                    ...
                    fetch("/api/comms/telegram/config", {method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})
                ...
                if (test) test.onclick = function() {
                    fetch("/api/comms/telegram/test",{method:"POST"}).then(function(r){return r.json();}).then(function(d){
                ...
                if (en) en.onclick = function() {
                    fetch("/api/comms/telegram/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:true})})
