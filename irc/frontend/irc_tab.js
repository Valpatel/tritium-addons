        ...
        window._tritiumEventBus.emit("panel:register-tab", {
            ...
            create: function(el) {
                el.innerHTML = "<div style=\"padding:8px;font-family:monospace;font-size:11px;color:#ccc\">"
                    + "<div style=\"color:#888888;font-size:12px;margin-bottom:8px\">IRC</div>"
                    + "<p style=\"color:#555;font-size:10px;margin-bottom:10px\">IRC bridge for text comms</p>"
                    + "<div style=\"color:#888;margin-bottom:6px;font-size:10px\">CONFIGURATION</div>"
                    + "<div class=\"cf-row\"><label class=\"cf-label\">SERVER</label><input type=\"text\" class=\"cf-input\" data-key=\"server\" placeholder=\"IRC server\"></div><div class=\"cf-row\"><label class=\"cf-label\">PORT</label><input type=\"text\" class=\"cf-input\" data-key=\"port\" placeholder=\"Port\"></div><div class=\"cf-row\"><label class=\"cf-label\">NICK</label><input type=\"text\" class=\"cf-input\" data-key=\"nick\" placeholder=\"Bot nick\"></div><div class=\"cf-row\"><label class=\"cf-label\">CHANNEL</label><input type=\"text\" class=\"cf-input\" data-key=\"channel\" placeholder=\"Alert channel\"></div><div class=\"cf-row\"><label class=\"cf-label\">USE_TLS</label><input type=\"checkbox\" class=\"cf-input\" data-key=\"use_tls\"></div>"
                    + "<div style=\"margin-top:10px;display:flex;gap:4px\">"
                    + "<button class=\"cf-btn\" data-action=\"save\" style=\"color:#888888;border-color:#888888\">SAVE</button>"
                ...
                if (save) save.onclick = function() {
                    ...
                    fetch("/api/comms/irc/config", {method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})
                ...
                if (test) test.onclick = function() {
                    fetch("/api/comms/irc/test",{method:"POST"}).then(function(r){return r.json();}).then(function(d){
                ...
                if (en) en.onclick = function() {
                    fetch("/api/comms/irc/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:true})})
