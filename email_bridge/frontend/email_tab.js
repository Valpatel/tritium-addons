        ...
        window._tritiumEventBus.emit("panel:register-tab", {
            ...
            create: function(el) {
                el.innerHTML = "<div style=\"padding:8px;font-family:monospace;font-size:11px;color:#ccc\">"
                    + "<div style=\"color:#dd4b39;font-size:12px;margin-bottom:8px\">EMAIL</div>"
                    + "<p style=\"color:#555;font-size:10px;margin-bottom:10px\">SMTP email notifications</p>"
                    + "<div style=\"color:#888;margin-bottom:6px;font-size:10px\">CONFIGURATION</div>"
                    + "<div class=\"cf-row\"><label class=\"cf-label\">SMTP_HOST</label><input type=\"text\" class=\"cf-input\" data-key=\"smtp_host\" placeholder=\"SMTP server\"></div><div class=\"cf-row\"><label class=\"cf-label\">SMTP_PORT</label><input type=\"text\" class=\"cf-input\" data-key=\"smtp_port\" placeholder=\"Port\"></div><div class=\"cf-row\"><label class=\"cf-label\">USERNAME</label><input type=\"text\" class=\"cf-input\" data-key=\"username\" placeholder=\"Username\"></div><div class=\"cf-row\"><label class=\"cf-label\">PASSWORD</label><input type=\"text\" class=\"cf-input\" data-key=\"password\" placeholder=\"Password\"></div><div class=\"cf-row\"><label class=\"cf-label\">FROM_ADDR</label><input type=\"text\" class=\"cf-input\" data-key=\"from_addr\" placeholder=\"From\"></div><div class=\"cf-row\"><label class=\"cf-label\">TO_ADDRS</label><input type=\"text\" class=\"cf-input\" data-key=\"to_addrs\" placeholder=\"To addresses\"></div>"
                    + "<div style=\"margin-top:10px;display:flex;gap:4px\">"
                    + "<button class=\"cf-btn\" data-action=\"save\" style=\"color:#dd4b39;border-color:#dd4b39\">SAVE</button>"
                ...
                if (save) save.onclick = function() {
                    ...
                    fetch("/api/comms/email/config", {method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})
                ...
                if (test) test.onclick = function() {
                    fetch("/api/comms/email/test",{method:"POST"}).then(function(r){return r.json();}).then(function(d){
                ...
                if (en) en.onclick = function() {
                    fetch("/api/comms/email/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:true})})
