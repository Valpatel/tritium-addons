...
// Signal communication addon tab
        ...
        window._tritiumEventBus.emit("panel:register-tab", {
            ...
            create: function(el) {
                el.innerHTML = "<div style=\"padding:8px;font-family:monospace;font-size:11px;color:#ccc\">"
                    + "<div style=\"color:#3a76f0;font-size:12px;margin-bottom:8px\">SIGNAL</div>"
                    + "<p style=\"color:#555;font-size:10px;margin-bottom:10px\">Signal E2E encrypted comms</p>"
                    + "<div style=\"color:#888;margin-bottom:6px;font-size:10px\">CONFIGURATION</div>"
                    + "<div class=\"cf-row\"><label class=\"cf-label\">SIGNAL_CLI_PATH</label><input type=\"text\" class=\"cf-input\" data-key=\"signal_cli_path\" placeholder=\"Path to signal-cli\"></div><div class=\"cf-row\"><label class=\"cf-label\">PHONE_NUMBER</label><input type=\"text\" class=\"cf-input\" data-key=\"phone_number\" placeholder=\"Registered number\"></div><div class=\"cf-row\"><label class=\"cf-label\">GROUP_ID</label><input type=\"text\" class=\"cf-input\" data-key=\"group_id\" placeholder=\"Alert group ID\"></div><div class=\"cf-row\"><label class=\"cf-label\">BRIDGE_ALERTS</label><input type=\"checkbox\" class=\"cf-input\" data-key=\"bridge_alerts\"></div>"
                    + "<div style=\"margin-top:10px;display:flex;gap:4px\">"
                    + "<button class=\"cf-btn\" data-action=\"save\" style=\"color:#3a76f0;border-color:#3a76f0\">SAVE</button>"
                ...
                if (save) save.onclick = function() {
                    ...
                    fetch("/api/comms/signal/config", {method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})
                ...
                if (test) test.onclick = function() {
                    fetch("/api/comms/signal/test",{method:"POST"}).then(function(r){return r.json();}).then(function(d){
                ...
                if (en) en.onclick = function() {
                    fetch("/api/comms/signal/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:true})})
