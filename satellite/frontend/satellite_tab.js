...
// Satellite communication addon tab
        ...
        window._tritiumEventBus.emit("panel:register-tab", {
            ...
            create: function(el) {
                el.innerHTML = "<div style=\"padding:8px;font-family:monospace;font-size:11px;color:#ccc\">"
                    + "<div style=\"color:#ff6600;font-size:12px;margin-bottom:8px\">SATELLITE</div>"
                    + "<p style=\"color:#555;font-size:10px;margin-bottom:10px\">Satellite comms beyond LOS</p>"
                    + "<div style=\"color:#888;margin-bottom:6px;font-size:10px\">CONFIGURATION</div>"
                    + "<div class=\"cf-row\"><label class=\"cf-label\">PROVIDER</label><input type=\"text\" class=\"cf-input\" data-key=\"provider\" placeholder=\"Provider\"></div><div class=\"cf-row\"><label class=\"cf-label\">MODEM_PORT</label><input type=\"text\" class=\"cf-input\" data-key=\"modem_port\" placeholder=\"Serial port\"></div><div class=\"cf-row\"><label class=\"cf-label\">BAUD_RATE</label><input type=\"text\" class=\"cf-input\" data-key=\"baud_rate\" placeholder=\"Baud rate\"></div><div class=\"cf-row\"><label class=\"cf-label\">BRIDGE_ALERTS</label><input type=\"checkbox\" class=\"cf-input\" data-key=\"bridge_alerts\"></div>"
                    + "<div style=\"margin-top:10px;display:flex;gap:4px\">"
                    + "<button class=\"cf-btn\" data-action=\"save\" style=\"color:#ff6600;border-color:#ff6600\">SAVE</button>"
                ...
                if (save) save.onclick = function() {
                    ...
                    fetch("/api/comms/satellite/config", {method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})
                ...
                if (test) test.onclick = function() {
                    fetch("/api/comms/satellite/test",{method:"POST"}).then(function(r){return r.json();}).then(function(d){
                ...
                if (en) en.onclick = function() {
                    fetch("/api/comms/satellite/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:true})})
