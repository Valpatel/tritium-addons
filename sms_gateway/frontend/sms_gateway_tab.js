...
// SMS Gateway communication addon tab
        ...
        window._tritiumEventBus.emit("panel:register-tab", {
            ...
            create: function(el) {
                el.innerHTML = "<div style=\"padding:8px;font-family:monospace;font-size:11px;color:#ccc\">"
                    + "<div style=\"color:#f22f46;font-size:12px;margin-bottom:8px\">SMS GATEWAY</div>"
                    + "<p style=\"color:#555;font-size:10px;margin-bottom:10px\">SMS via Twilio or GSM modem</p>"
                    + "<div style=\"color:#888;margin-bottom:6px;font-size:10px\">CONFIGURATION</div>"
                    + "<div class=\"cf-row\"><label class=\"cf-label\">PROVIDER</label><input type=\"text\" class=\"cf-input\" data-key=\"provider\" placeholder=\"twilio or gsm_modem\"></div><div class=\"cf-row\"><label class=\"cf-label\">ACCOUNT_SID</label><input type=\"text\" class=\"cf-input\" data-key=\"account_sid\" placeholder=\"Twilio SID\"></div><div class=\"cf-row\"><label class=\"cf-label\">AUTH_TOKEN</label><input type=\"text\" class=\"cf-input\" data-key=\"auth_token\" placeholder=\"Auth token\"></div><div class=\"cf-row\"><label class=\"cf-label\">FROM_NUMBER</label><input type=\"text\" class=\"cf-input\" data-key=\"from_number\" placeholder=\"Sender\"></div><div class=\"cf-row\"><label class=\"cf-label\">ALERT_NUMBERS</label><input type=\"text\" class=\"cf-input\" data-key=\"alert_numbers\" placeholder=\"Recipients\"></div>"
                    + "<div style=\"margin-top:10px;display:flex;gap:4px\">"
                    + "<button class=\"cf-btn\" data-action=\"save\" style=\"color:#f22f46;border-color:#f22f46\">SAVE</button>"
                ...
                if (save) save.onclick = function() {
                    ...
                    fetch("/api/comms/sms_gateway/config", {method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})
                ...
                if (test) test.onclick = function() {
                    fetch("/api/comms/sms_gateway/test",{method:"POST"}).then(function(r){return r.json();}).then(function(d){
                ...
                if (en) en.onclick = function() {
                    fetch("/api/comms/sms_gateway/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:true})})
